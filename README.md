...
- [RenderingOutline](#renderingoutline)
  * [渲染的一些参考资料](#---------)
  * [全篇第一个"Why, What, How"](#------why--what--how-)
  * [第一章 基础](#------)
    + [1.渲染管线简介/Rendering Pipeline](#1-------rendering-pipeline)
    + [2.数学基础/Math](#2-----math)
    + [3.纹理/Texture](#3---texture)
    + [4.图形API/Graphics API](#4--api-graphics-api)
    + [5.图形API坐标系对比/Coordinate System](#5--api------coordinate-system)
  * [第二章 光照基础](#--------)
    + [1.颜色空间/Color Space](#1-----color-space)
    + [2.模型与材质/Model & Materials](#2------model---materials)
    + [3.基础hlsl函数介绍/HLSL Functions](#3--hlsl-----hlsl-functions)
    + [4.传统经验光照模型/Empirical Shading Model](#4---------empirical-shading-model)
    + [5.凹凸贴图/Bump Mapping](#5-----bump-mapping)
    + [6.伽马校正/Gamma Correction](#6-----gamma-correction)
    + [7.LDR&HDR](#7ldr-hdr)
    + [8.FlowMap的实现](#8flowmap---)
  * [第三章 进阶应用](#--------)
    + [1.深度与模板测试/Depth Testing & Stencil Testing](#1--------depth-testing---stencil-testing)
    + [2.混合模式及剔除/Blending & Culling](#2--------blending---culling)
    + [3.曲面细分着色器与几何着色器/ts & gs](#3--------------ts---gs)
    + [4.延迟渲染原理介绍/Deferred Rendering](#4---------deferred-rendering)
    + [5.early-Z & Z-Prepass](#5early-z---z-prepass)
    + [6.纹理压缩](#6----)
    + [7.现代移动端的TBR和TBR渲染管线以及Cbuffer](#7------tbr-tbr------cbuffer)
    + [7.1 名词解释](#71-----)
    + [7.2 IMR & TBR](#72-imr---tbr)
    + [7.3 TBR的优化策略](#73-tbr-----)
    + [7.4 Cbuffer](#74-cbuffer)
    + [8.DepthPeeling 深度剥离](#8depthpeeling-----)
  * [第四章 高级扩展](#--------)
    + [1.Bloom算法](#1bloom--)
    + [2.SSAO算法](#2ssao--)
    + [3.阴影的各类实现方法](#3---------)
    + [4.抗锯齿之FXAA、MSAA](#4----fxaa-msaa)
    + [5.DOF景深算法](#5dof----)
    + [6.运动模糊](#6----)
    + [7.雾效实现方法](#7------)
    + [8.基于屏幕空间的溶解](#8---------)
  * [第五章 物理世界](#--------)
    + [1.PBR](#1pbr)
    + [1.1 辐射度量学/Radiometry](#11-------radiometry)
    + [1.2 BRDF](#12-brdf)
    + [1.3 PBR实现算法](#13-pbr----)
    + [2.光线追踪](#2----)
    + [3.Real-time Environment Mapping](#3real-time-environment-mapping)
    + [4.Real-time GLobal Illumination (in 3D)](#4real-time-global-illumination--in-3d-)
    + [5.Real-time GLobal Illumination (screen space)](#5real-time-global-illumination--screen-space-)
    + [6.体渲染](#6---)
    + [7.体积云的生成](#7------)
    + [8.水体渲染](#8----)
    + [9.毛发模拟](#9----)
    ...
    + [11.皮肤渲染](#11----)
  * [第六章 卡通渲染](#--------)
    + [1.NPR与卡通渲染的风格流派](#1npr----------)
    + [2.卡通渲染描边方案](#2--------)
    + [3.卡通渲染着色方案](#3--------)
    + [4.卡通渲染案例讲解](#4--------)
  * [第七章 GPU逻辑管线](#----gpu----)
    + [1.逻辑管线简介/GPU Logical Pipeline](#1-------gpu-logical-pipeline)
    + [2.GPU硬件架构十三问/GPU Q&A](#2gpu--------gpu-q-a)
    + [3.光栅化与透视矫正插值/Rasterization & Perspective-Correct Interpolation](#3-----------rasterization---perspective-correct-interpolation)
    + [4.if，以及分支/if, and branch](#4if------if--and-branch)
    + [5.基于GPU架构的渲染优化建议](#5--gpu---------)
  * [第八章 其他](#------)
    + [1.体素](#1--)
    + [2.三维旋转：欧拉角、四元数、旋转矩阵、轴角](#2--------------------)
    + [2.1 欧拉角基础](#21------)
    + [2.2 四元数基础](#22------)
    + [2.3 转换](#23---)
    + [3.球谐光照](#3----)
...
四元数的模长、加减法与复数类似，但四元数的乘法是有左乘与右乘之分的，我们有公式：

Graßmann积：𝑞1𝑞2 = [𝑎𝑒 − **v** · **u**, 𝑎**u** + 𝑒**v** + **v** × **u**].

纯四元数以及共轭四元数的定义也与虚数类似，四元数的逆与矩阵类似。对于一个单位四元数，它的逆等于它的共轭。
